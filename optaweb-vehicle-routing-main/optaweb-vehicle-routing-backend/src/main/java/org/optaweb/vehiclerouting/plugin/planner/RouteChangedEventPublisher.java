    ...
    private static List<ShallowRoute> routes(VehicleRoutingSolution solution) {
        ...
        for (PlanningVehicle vehicle : solution.getVehicleList()) {
            ...
            for (PlanningVisit visit : vehicle.getFutureVisits()) {
                ...
                routeDistance += visit.distanceFromPreviousStandstill();
                if (visit.getNextVisit() == null) {
                    routeDistance += visit.distanceToDepot();
                }

            }
            /*
             * if (solution.getVisitList().size() > 0) {
             * routeDistance = calculateDistance(vehicle.getNextVisit(), true);
             * }
             */
            routes.add(new ShallowRoute(vehicle.getId(), depot.getId(), visits, routeDistance));
        }
        return routes;
    }

    private static double calculateDistance(PlanningVisit next, boolean first) {
        if (next != null) {
            if (first) {
                return next.distanceToDepot() + calculateDistance(next.getNextVisit(), false);
            }
            return next.distanceFromPreviousStandstill() + calculateDistance(next.getNextVisit(), false);
        } else {
            return 0;
        }
    }
